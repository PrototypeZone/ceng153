...
46. The lower STM32 can also be programmed with ```J31``` enabling LEDs on ```PB12``` (red), ```PB13``` (green), and ```PB14``` (green).
47. This STM32F103 microcontroller based platform ([Schematic](CBA_2021_04_24.pdf), [Gerbers](CBA_2021_04_24_Gerbers.png))
...
48. This platform is deprecated due to STM32 shortages and OLED reliability issues. We have moved to a RP2040 platform.
49. Both BOOT jumpers are on the 0 side (versus 1).   
...
50. J Headers - isolation of many components is possible, note that uSD disconnect is simply removing the card:   

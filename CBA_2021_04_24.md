...
44. The potentiometers on the joystick are read by ```PA0``` and ```PA2```.
45. ```PA1``` is the upper servo pin (yellow), ground (brown) is closer to the joystick - example doesn't work with first test.   
46. This STM32F103 microcontroller based platform ([Schematic](CBA_2021_04_24.pdf), [Gerbers](CBA_2021_04_24_Gerbers.png))
...
an analog joystick, battery and a connector for a servo. As such the plan is for this document to provide more examples in the future (please share any successes with me), ideally to easily test the OLED.
47. This platform is deprecated due to STM32 shortages and OLED reliability issues. We have moved to a RP2040 platform.
48. Both BOOT jumpers are on the 0 side (versus 1).   
...
49. J Headers - isolation of many components is possible, note that uSD disconnect is simply removing the card:   

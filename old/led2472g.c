...
/** @brief Converts an integer to an LED matrix pattern arrow.
 */
void ShConvertIntegerToPattern(int s, uint16_t image[8][8], uint16_t colorText, uint16_t colorBackground)
{
    int i=0;
    int j,k;
    int r=s/15;
...
/** @brief Rotates an image array
 *  @param int rotation angle 90, 180, 270, -90, -180, -270
 */
void ShRotateImage(int angle, uint16_t image[8][8])
{
    uint16_t tabAux[8][8];
    ...
    for(int row=0; row <8 ; row++)
    {
        for(int column=0 ; column <8 ; column++)
        {
            switch(angle)
            {
                case   90:
                case -270:
                    tabAux[7 - column][row] = image[row][column];
                    break;
                case  180:
                case -180:
                    tabAux[7 - row][7 - column] = image[row][column];
                    break;
                case  270:
                case  -90:
                    tabAux[column][7 - row] = image[row][column];
                    break;
                default:
                    tabAux[row][column] = image[row][column];
            }
        }
    }
    ...
    for(int row=0; row <8 ; row++)
    {
        for(int column=0 ; column <8 ; column++)
        {
            image[row][column] = tabAux[row][column];
        }
    }
}
